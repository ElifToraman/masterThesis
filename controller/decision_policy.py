...
class DecisionPolicy:
    ...
    Output:
      - Best cluster for Knative Service deployment
    """
    ...
    def decide(
        ...
        *,
        ...
        if satisfied_candidates:
            ...
            mode = "best-effort"
            reason = (
                f"No feasible cluster satisfies all intent objectives. "
                f"Selected {selected.cluster_name} as best-effort."
            )
        else:
            selected = None
            mode = "no-feasible-cluster"
            reason = "No cluster has enough monitoring and benchmark evidence."
        ...
        return PlacementDecision(
            ...
            selected_cluster=(
                selected.cluster_name if selected is not None else None
            ),
            decision_mode=mode,
            reason=reason,
    ...
    def _build_candidate(
        ...
        *,
        ...
        worker_nodes = [
            ...
            if node.node_role.lower() not in {
                "control-plane",
                "master",
            }
        ]
        ...
        return ClusterDecisionCandidate(
            ...
            vm_ssh_latency_ms=(
                vm.ssh_latency_ms if vm is not None else None
            ),
    ...
    def _score(
        ...
        *,
        ...
        Main priority:
          1. warm p95 latency
          2. warm average latency
          3. current node CPU/memory load
        ...
        avg = benchmark_average_latency_ms
        if avg is None:
            avg = p95

        first = benchmark_first_invocation_latency_ms or 0.0
        deploy = benchmark_deployment_duration_ms or 0.0
        ...
        *,
        ...
        with self.benchmark_file.open("r", encoding="utf-8") as file:
            for line in file:
                ...
                # Some older benchmark records may not contain function_version.
                record_version = record.get("function_version")
                if record_version is not None and record_version != function_version:
                    continue
...
cat > controller/decision_policy.py <<'PY'
from __future__ import annotations
